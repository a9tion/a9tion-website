...
### 3. `daily_ops_makuake` シートのテストデータを削除

ダッシュボードの「日次オペレーション」セクションはこのシートを参照している。同様にテストデータをクリアする。

1. `daily_ops_makuake` シートを開く
2. 1行目のヘッダー行（`date | store | traffic | demos | first_time | existing | memo`）は残す
3. 2行目以降のテストデータを選択し、行ごと削除
4. ヘッダー行だけが残っている状態にして保存

> 本番投入前にダッシュボード（`dashboard.html`）を開き、フィードバック・日次オペレーションともに件数が0件になっていることを確認する。
> ダッシュボードのログインには [クライアント別ダッシュボードURL一覧](#クライアント別ダッシュボードurl一覧) のPIN（さわれるMakuakeは`739205`）を使う。
